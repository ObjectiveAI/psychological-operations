...
# psychological-operations-x-api-mcp build (its own src + Cargo.toml +
# the workspace Cargo.lock; no transitive workspace deps).
#
# Usage:
...
# Parse --target and --release from args. Default target is the rustc
# host triple — this binary runs on the operator's machine (via embed
# into the CLI), not inside a Docker container.
TARGET=""
PROFILE="debug"
prev_was_target=0
for arg in "$@"; do
