...
/// postgres-stored Discord queue (and, later, the serenity-backed Discord
/// REST client for read/write tools).
///
...
/// JSON-object header on every connect. See `crate::discord_api::session` for
/// the source-resolution contract.
#[derive(Parser)]
#[command(name = "psychological-operations-discord-mcp")]
struct Args {
