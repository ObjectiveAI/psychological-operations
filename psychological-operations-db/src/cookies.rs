//! On-disk reader for CEF cookie state — the one place this crate
//! touches SQLite instead of postgres (Chromium's cookie jar is its own
//! on-disk SQLite, not ours to move).
//!
//! The runtime browser process owns its own live cookie view via CEF's
//! `CookieManager`. This module is for *external* callers (CLI, SDK
//! `Client`) that need to ask "who is signed in?" without being
//! in-process — they read the CEF cookies SQLite store directly off disk
...
/// `twid` is shaped `u%3D<numeric-id>` (URL-encoded `u=<id>`). Pull out
/// the digits. Match both the URL-encoded and decoded prefixes —
/// different consumers of the cookie store may or may not URL-decode the
/// raw value for us.
...
/// failure modes (I/O, SQLite, JSON parse of `Local State`, key material
/// missing, DPAPI failure, AES-GCM failure). Callers pattern-match on
/// the variants to disambiguate setup states from real failures.
#[derive(Debug)]
pub enum CookiesError {
    ...
    /// (today: anything other than `v10` — `v11` is Chromium's app-bound
    /// scheme and not implemented here yet). String holds the prefix for
    /// diagnostics.
...
/// Look up the X user-id currently signed in for the CEF context whose
/// cache lives at `<state_dir>/browser/cef-root/<cache_subdir>/` by
/// reading the on-disk CEF cookies store. Returns `Ok(None)` if the
/// cookies DB doesn't exist yet (browser never opened in this context),
/// the DB has no `twid` row, or the `twid` value doesn't parse as a
/// numeric user-id. Real I/O / decrypt failures bubble up as `Err`.
pub async fn signed_in_x_user_id(
    ...
    // `read_only` + `immutable` together tell SQLite the file won't
    // change underneath it and skip all OS-level locking, which is the
    // only way to coexist with Chromium's FILE_SHARE_NONE handle on
    // Windows (a plain fs::copy or OpenOptions::read fails with
...
/// Parse `Local State` JSON → unwrap `os_crypt.encrypted_key` → strip
/// the `"DPAPI"` literal prefix → DPAPI-unprotect → raw 32-byte
/// AES-256-GCM key.
#[cfg(windows)]
...
/// then strip Chromium's 32-byte SHA-256 integrity prefix that recent
/// Chromium versions (≥ 127-ish) prepend to the plaintext before
/// encryption (the hash of `host_key + name`, used to detect cookie-row
/// tampering).
///
...
fn decrypt_value(key: &[u8], blob: &[u8]) -> Result<Vec<u8>, CookiesError> {
    ...
    // Strip the 32-byte SHA-256 integrity prefix if present. Older
    // Chromium builds (< 127-ish) didn't prepend it — guard by length so
    // we don't truncate a short plaintext into nothing on the legacy
    // path.
