...
1. **Capture** — A Chrome extension running on x.com sends every tweet ID it sees in the For You feed to a local queue (`for_you_queue`). The extension only ever sees IDs; nothing leaves the machine until the next run.
...
- `for_you` — the algorithmic timeline. Captured passively by the Chrome extension as the psyop's account browses x.com. Optional `priority` and `filter`.
...
- **One X developer app, shared billing.** All psyops authenticate against a single X Project + App. Credentials live in `~/.psychological-operations/x_app.json` and are captured once via the Chrome extension's setup flow. Every API call — search, hydrate, like, retweet, regardless of which psyop initiated it — deducts from this one credit pool.
...
- **`psychological-operations-chrome-extension`** — MV3 Chrome extension. Walks the For You DOM on `x.com` and pipes tweet IDs to the CLI over Chrome native-messaging. Also captures the `x_app.json` credentials during initial setup.
- **`psychological-operations-chrome`** — Bundles a pinned Chrome for Testing build + a packed/signed CRX of the extension; embedded into the CLI binary at compile time and extracted into per-psyop profile directories at runtime.
- **`crx-pack`** — Build-time tool that packs the unpacked extension into a signed CRX3 with a deterministic key, so the same extension ID is reproduced across builds.
