...
- **`psychological-operations-chromium-extension`** — MV3 Chromium extension. Walks the For You DOM on `x.com` and pipes tweet IDs to the CLI over Chromium native-messaging. Also captures the `x_app.json` credentials during initial setup.
- **`psychological-operations-chromium`** — Bundles a pinned upstream Chromium snapshot + a packed/signed CRX of the extension; embedded into the CLI binary at compile time and extracted into per-psyop profile directories at runtime.
