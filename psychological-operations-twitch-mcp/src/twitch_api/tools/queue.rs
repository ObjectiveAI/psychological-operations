//! Queue tools — read + dequeue the per-agent Twitch ingest queue.
//!
//! DB-only (no Twitch API call) and quota-free. The queue is keyed by agent
...
//! psyop-group item carrying all its `(channel_login, message_id, score)`
//! tuples; each operator-flagged row (a daemon hook match / manual enqueue, no
//! `run_id`) is its own item. `count`/`offset` window the item list. The agent's
//! own `agent_tag` is never echoed, and `queued_at` is rendered RFC 3339.
//!
//! A Twitch message is addressed by BOTH the channel login and the message id,
//! so `mark_handled` takes `(channel_login, message_id)` pairs.
...
struct PsyopGroup {
    ...
    /// `(channel_login, message_id, score)` for every message this run delivered.
    messages: Vec<(String, String, f64)>,
}

/// A single operator-flagged message (a daemon hook match / manual enqueue).
#[derive(serde::Serialize)]
struct OperatorItem {
...
pub struct MarkHandledRequest {
    #[schemars(description = "The messages to remove from the queue, each by its \
                             channel login + message id.")]
    pub messages: Vec<MarkHandledMessage>,
}

#[derive(Debug, serde::Deserialize, schemars::JsonSchema)]
pub struct MarkHandledMessage {
    #[schemars(description = "The channel login the message is in.")]
    pub channel_login: String,
    #[schemars(description = "The message's IRCv3 id.")]
    pub message_id: String,
}

#[tool_router(router = queue_tools, vis = "pub")]
impl PsychologicalOperationsTwitchMcp {
    #[tool(name = "read_queue", description = "Read pending Twitch messages from the queue.")]
    async fn read_queue(
        &self,
        ...
        finish(
            async move {
                ...
                let entries = self.db.twitch_queue_list(&tag).await?;
                ...
                for e in entries {
                    match e.run_id {
                        Some(run_id) => {
                            let message = (e.channel_login, e.message_id, e.score.unwrap_or(0.0));
        ...
        finish(
            async move {
                let keys: Vec<(String, String)> = req
                    .messages
                    .iter()
                    .map(|m| (m.channel_login.clone(), m.message_id.clone()))
                    .collect();
                let missing = self.db.twitch_queue_delete_many(&tag, &keys).await?;
