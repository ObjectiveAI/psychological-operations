...
//!    `http::request::Parts` (per [`crate::twitch_api::session::HEADER_ARGUMENTS`]),
...
//!
//! This is a near-verbatim copy of the Discord MCP's manager (only the server
//! type + the default message length differ); see that crate for the deeper
//! commentary.
...
pub struct HeaderSessionManager {
    ...
    /// worker.
    service: PsychologicalOperationsTwitchMcp,
}
    ...
    let request = Request {
        ...
        params: InitializeRequestParams {
            ...
            client_info: Implementation {
                name: "psychological-operations-twitch-mcp-restore-stub".into(),
                title: None,
                version: "0".into(),
                description: None,
                icons: None,
...
fn extract_session_state(message: &ClientJsonRpcMessage) -> Result<SessionState, String> {
    ...
    // Per-tool cost overrides (`quota_usage_<tool>`), one per metered tool in
    // `ToolName::ALL`; each falls back to `DEFAULT_TOOL_COST`.
