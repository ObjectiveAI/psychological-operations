//! Unit tests for the codegen'd X v2 media types.
//!
//! Verifies the `MediaUnion` tagged enum round-trips against the X v2
//! wire shapes — photo, video, animated_gif — even though each subtype
//! `#[serde(flatten)]`s `Media` (whose `type_` field is demoted to
//! `Option<String>` precisely so the enum tag can consume the wire
//! `type` field without the inner `Media` struct failing to find it).
//!
//! `MediaUnion` is generated (`types/media.rs`, do-not-edit), so these
//! live in this hand-written sibling module rather than inline.

use crate::x::types::media::MediaUnion;
