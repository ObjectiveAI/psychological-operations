...
/// A home-timeline ingestion source on a psyop. Reads the agent's
/// `/2/users/{id}/timelines/reverse_chronological` feed (posts from accounts
/// the agent follows), paginating up to `max_posts`. Sorts like a query
/// (via `SortBy`), not interwoven like for_you.
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
pub struct Timeline {
    /// The agent whose home timeline this reads, acting as its auth
