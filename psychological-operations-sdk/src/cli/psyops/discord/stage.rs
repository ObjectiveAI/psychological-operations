...
/// (threshold, invert, text/images/videos hints, the function /
...
/// One pass over the surviving message set. Tagged on `"type"`:
///
/// - **`bare`** — pass-through. Assigns every message a flat 1.0
///   score and applies the shared `output_top` cap. No
///   objectiveai call, no threshold, no per-call invert /
///   text / images / videos hints.
/// - **`function`** — the objectiveai scoring shape: function +
///   profile + strategy + invert + text / images / videos +
///   output_threshold + the shared output_top. The shared
///   `output_top` lands at top-level JSON via `#[serde(flatten)]`
///   on the embedded `StageBase`.
///
/// PsyOp carries a `Vec<Stage>` so multi-stage pipelines can mix
/// the two.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
...
pub enum Stage {
    ...
    Function {
        ...
        #[serde(default)]
        invert: bool,
        /// If `false`, scored messages are sent to the function
        /// with an empty `text` string regardless of what was
        /// ingested (the message id is always included). Defaults
        /// to `true`.
        ...
        #[serde(default = "default_true")]
        videos: bool,
        /// Drop messages scoring below this threshold before they
...
/// tagged on `"type"` + `"value"`.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(tag = "type", content = "value", rename_all = "snake_case")]
pub enum OutputTop {
    /// Keep the top `value` messages after scoring (and, for the
    ...
    Fixed(u64),
    /// Keep the top `ceil(N · value)` messages. `value` is in
    /// `[0.0, 1.0]` — e.g. `0.25` = top quarter.
    Fraction(f64),
    /// Python expression. The `input` global is a list of dicts mirroring a
    /// Discord message plus `score: float` (the just-computed stage score).
    /// Its trailing expression must evaluate to a non-negative integer (or a
    /// float that round-trips cleanly to one). The result is the absolute
